...
export function domAction(input={action:'capture',walkFrames:true}) {
 ...
 const visible=el=>{const win=el.ownerDocument.defaultView;const style=win.getComputedStyle(el);return !el.closest('[hidden],[aria-hidden="true"]')&&style.display!=='none'&&style.visibility!=='hidden'&&el.getClientRects().length>0};
 const eligible=el=>{const type=(el.type||'').toLowerCase();return !el.disabled&&(!el.readOnly||input.action==='capture')&&visible(el)&&!['hidden','password','submit','button','reset','image'].includes(type)&&!secret.test([el.name,el.id,label(el),el.autocomplete].join(' '))&&!gate.test([el.name,el.id,label(el)].join(' '))&&!/^(one-time-code|cc-)/i.test(el.autocomplete||'')&&!el.closest('[data-sitekey],.g-recaptcha,.h-captcha,.cf-turnstile,[id*="captcha" i],[class*="captcha" i],[id*="turnstile" i],[class*="turnstile" i]')};
 const docs=[];
